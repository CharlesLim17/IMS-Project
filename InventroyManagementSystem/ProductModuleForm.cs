    ...
    public partial class ProductModuleForm : Form
    {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                if (MessageBox.Show("Are you sure you want to save this product?", "Saving Record", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    cm.Parameters.AddWithValue("@pqty", Convert.ToInt64(txtPQty.Text));
                    cm.Parameters.AddWithValue("@pprice", Convert.ToInt64(txtPPrice.Text));
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    cm = new SqlCommand("UPDATE tbProduct SET pname=@pname, pqty=@pqty, pprice=@pprice, pdescription=@pdescription, pcategory=@pcategory WHERE pid LIKE '" + lvlPid.Text + "' ", con);
                    cm.Parameters.AddWithValue("@pname", txtPName.Text);
                    cm.Parameters.AddWithValue("@pqty", Convert.ToInt64(txtPQty.Text));
                    cm.Parameters.AddWithValue("@pprice", Convert.ToInt64(txtPPrice.Text));
                    cm.Parameters.AddWithValue("@pdescription", txtPDes.Text);
